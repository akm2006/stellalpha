...
import { LayoutDashboard, Star, Loader2, Wallet, X, Terminal, Command, ShieldCheck } from "lucide-react";
...
export default function LocalNavbar() {
  ...
  return (
    <header className="absolute top-0 inset-x-0 z-50 bg-[#050505]/95 backdrop-blur-sm border-b border-white/10 h-16">
      <div className="max-w-[1920px] mx-auto h-full flex items-center justify-between pl-6 pr-6 md:pr-8">
        ...
        <div className="flex items-center gap-6 h-full">
          ...
          <button
            className="md:hidden flex flex-col items-center justify-center w-10 h-10 border border-white/10 bg-white/5 text-slate-300 hover:text-white transition-all rounded-sm"
            ...
            <nav className="flex flex-col">
              {navigationItems.map((item, idx) => (
                <Link
                  ...
                  className="flex items-center gap-4 px-6 py-4 border-b border-white/5 hover:bg-emerald-500/5 group transition-colors"
                >
                  <span className="text-xs font-mono text-emerald-500/50">0{idx + 1}</span>
                  <span className="text-sm font-mono uppercase tracking-wider text-slate-300 group-hover:text-emerald-400">
                    {item.label}
                  </span>
