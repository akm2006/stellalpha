...
export function GradientButton({ title, icon, gradientFrom, gradientTo, onClick, className }: GradientButtonProps) {
    return (
        <li
            onClick={onClick}
            style={{ '--gradient-from': gradientFrom, '--gradient-to': gradientTo } as React.CSSProperties}
            className={cn(
                "relative w-[60px] h-[60px] bg-white shadow-lg rounded-full flex items-center justify-center transition-all duration-500 hover:w-[180px] hover:shadow-none group cursor-pointer",
                className
            )}
        >
...
// Full menu component using the individual button for demonstration
