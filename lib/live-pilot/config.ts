...
function parseWalletConfig(slot: 'A' | 'B', errors: string[]): PilotWalletConfigInternal | null {
  ...
  const anyConfigured = Boolean(publicKey || secret || starTrader || process.env[`PILOT_WALLET_${slot}_ALIAS`]);
  if (!anyConfigured) {
    return null;
  }
  ...
  return {
    ...
    maxTradeBuypowerPct: parseNumber('PILOT_MAX_TRADE_BUYPOWER_PCT', 0.5, errors),
    buyMaxPriceImpactPct: parseNumber('PILOT_BUY_MAX_PRICE_IMPACT_PCT', 0.08, errors),
...
export function toLivePilotConfigSummary(config: LivePilotConfig): LivePilotConfigSummary {
  return {
    operatorWallets: config.operatorWallets,
    errors: config.errors,
    wallets: config.wallets.map(({ secret: _secret, ...wallet }) => wallet),
  };
}
