...
import { enqueueParsedTxMessages } from '@/lib/ingestion/parsed-tx-queue';
import { getCachedNonTradeSignatures } from '@/lib/repositories/non-trade-signatures.repo';
...
//   2. Compare against `trades` table and recent non-trade cache
//   3. Queue unresolved recent signatures into the durable parsed-tx queue
//
 ...
 */
async function getRecentSignatures(wallet: string, limit: number) {
  ...
  const entries: { signature: string; err: any; blockTime?: number | null }[] = json.result || [];
  return entries.filter(e => e.err === null).map((entry) => ({
...
export async function GET(request: Request) {
  ...
  try {
    ...
    let totalQueued = 0;
    const walletSummaries: { wallet: string; queued: number }[] = [];
    ...
    for (const trader of traders) {
      ...
      try {
        ...
        const { data: knownTrades } = await supabase
          .from('trades')
          .select('signature')
          .in('signature', recentSigs.map((entry) => entry.signature));

        const knownSigs = new Set((knownTrades || []).map(t => t.signature));
        const cachedNonTrades = await getCachedNonTradeSignatures(
          wallet,
          recentSigs.map((entry) => entry.signature),
          new Date().toISOString()
        );
        const unseenSigs = recentSigs.filter((entry) => !knownSigs.has(entry.signature));
        const missingSigs = unseenSigs.filter((entry) => !cachedNonTrades.has(entry.signature));
        ...
        totalQueued += missingSigs.length;
        walletSummaries.push({ wallet: wallet.slice(0, 8), queued: missingSigs.length });

        console.log(
          `[RECONCILE] ${wallet.slice(0, 8)}...: queued ${missingSigs.length} unresolved sig(s)`
        );
    ...
    console.log(
      ...
      `Wallets: ${traders.length} | Queued: ${totalQueued}`
    );
